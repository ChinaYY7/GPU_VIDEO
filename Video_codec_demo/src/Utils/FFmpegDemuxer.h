/*
* Copyright 2017-2018 NVIDIA Corporation.  All rights reserved.
*
* Please refer to the NVIDIA end user license agreement (EULA) associated
* with this source code for terms and conditions that govern your use of
* this software. Any use, reproduction, disclosure, or distribution of
* this software and related documentation outside the terms of the EULA
* is strictly prohibited.
*
*/
#pragma once

extern "C" {
#include <libavformat/avformat.h>
#include <libavformat/avio.h>
#include <libavcodec/avcodec.h>
}
#include "NvCodecUtils.h"
#include "stdio.h"

//---------------------------------------------------------------------------
//! \file FFmpegDemuxer.h 
//! \brief Provides functionality for stream demuxing
//!
//! This header file is used by Decode/Transcode apps to demux input video clips before decoding frames from it. 
//---------------------------------------------------------------------------
/**
* @brief libavformat wrapper class. Retrieves the elementary encoded stream from the container format.
*/

class FFmpegDemuxer {
private:
    AVFormatContext *fmtc = NULL;   // 封装格式上下文的结构体，也是统领全局的结构体，保存了视频文件封装格式的相关信息
    AVIOContext *avioc = NULL;
    AVPacket pkt, pktFiltered; /*!< AVPacket stores compressed data typically exported by demuxers and then passed as input to decoders *///存储1帧压缩编码数据
    AVBSFContext *bsfc = NULL;
    AVFrame *pFrame = NULL;
    AVCodecContext  *pCodecCtx = NULL;  //编码器上下文结构体，保存了视频（音频）编解码相关信息
    AVCodec         *pCodec = NULL;  	//解码器，每种视频（音频）编解码器（例如H.264解码器）对应一个该结构体

    int iVideoStream;
    bool bMp4H264, bMp4HEVC, bMp4MPEG4;
    AVCodecID eVideoCodec;
    AVPixelFormat eChromaFormat;
    int nWidth, nHeight, nBitDepth, nBPP, nChromaHeight;
    double timeBase = 0.0;

    uint8_t *pDataWithHeader = NULL;

    unsigned int frameCount = 0;
    char filename[512];
    unsigned int key_frameCount = 0;
    unsigned int key_frame_after_count = 0;
    

    uint64_t Pframe_interval_pts = 0, start_Pframe_pts = 0, end_Pframe__pts = 0;
    uint64_t Kframe_pts, Pframe_pts;
    bool start_P_sta = false, Exit_probe = false;

public:
    class DataProvider {
    public:
        virtual ~DataProvider() {}
        virtual int GetData(uint8_t *pBuf, int nBuf) = 0;
    };

private:

    /**
    *   @brief  Private constructor to initialize libavformat resources.
    *   @param  fmtc - Pointer to AVFormatContext allocated inside avformat_open_input()
    */
    FFmpegDemuxer(AVFormatContext *fmtc) : fmtc(fmtc) {
        if (!fmtc) {
            NV_LOG(ERROR) << "No AVFormatContext provided.";
            return;
        }

        NV_LOG(INFO) << "Media format: " << fmtc->iformat->long_name << " (" << fmtc->iformat->name << ")";

        //3.读取一部分视音频数据并且获得一些相关的信息
        ck(avformat_find_stream_info(fmtc, NULL));
        //4.每个视频文件中有多个流（视频流、音频流、字幕流等，而且可有多个），这里获取其中的视频流
        iVideoStream = av_find_best_stream(fmtc, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
        if (iVideoStream < 0) {
            NV_LOG(ERROR) << "FFmpeg error: " << __FILE__ << " " << __LINE__ << " " << "Could not find stream in input file";
            return;
        }

        //fmtc->streams[iVideoStream]->need_parsing = AVSTREAM_PARSE_NONE;
        eVideoCodec = fmtc->streams[iVideoStream]->codecpar->codec_id;                  //获得编码器id
        nWidth = fmtc->streams[iVideoStream]->codecpar->width;                          //视频分辨率宽
        nHeight = fmtc->streams[iVideoStream]->codecpar->height;                        //视频分辨率高
        eChromaFormat = (AVPixelFormat)fmtc->streams[iVideoStream]->codecpar->format;   //视频色彩空间
        AVRational rTimeBase = fmtc->streams[iVideoStream]->time_base;                  //视频帧时间戳
        timeBase = av_q2d(rTimeBase);                                                   //视频帧率

        // Set bit depth, chroma height, bits per pixel based on eChromaFormat of input
        switch (eChromaFormat)
        {
        case AV_PIX_FMT_YUV420P10LE:
            nBitDepth = 10;
            nChromaHeight = (nHeight + 1) >> 1;
            nBPP = 2;
            break;
        case AV_PIX_FMT_YUV420P12LE:
            nBitDepth = 12;
            nChromaHeight = (nHeight + 1) >> 1;
            nBPP = 2;
            break;
        case AV_PIX_FMT_YUV444P10LE:
            nBitDepth = 10;
            nChromaHeight = nHeight << 1;
            nBPP = 2;
            break;
        case AV_PIX_FMT_YUV444P12LE:
            nBitDepth = 12;
            nChromaHeight = nHeight << 1;
            nBPP = 2;
            break;
        case AV_PIX_FMT_YUV444P:
            nBitDepth = 8;
            nChromaHeight = nHeight << 1;
            nBPP = 1;
            break;
        case AV_PIX_FMT_YUV420P:
        case AV_PIX_FMT_YUVJ420P:
        case AV_PIX_FMT_YUVJ422P:   // jpeg decoder output is subsampled to NV12 for 422/444 so treat it as 420
        case AV_PIX_FMT_YUVJ444P:   // jpeg decoder output is subsampled to NV12 for 422/444 so treat it as 420
            nBitDepth = 8;
            nChromaHeight = (nHeight + 1) >> 1;
            nBPP = 1;
            break;
        default:
            NV_LOG(WARNING) << "ChromaFormat not recognized. Assuming 420";
            nBitDepth = 8;
            nChromaHeight = (nHeight + 1) >> 1;
            nBPP = 1;
        }

        bMp4H264 = eVideoCodec == AV_CODEC_ID_H264 && (
                !strcmp(fmtc->iformat->long_name, "QuickTime / MOV") 
                || !strcmp(fmtc->iformat->long_name, "FLV (Flash Video)") 
                || !strcmp(fmtc->iformat->long_name, "Matroska / WebM")
            );
        bMp4HEVC = eVideoCodec == AV_CODEC_ID_HEVC && (
                !strcmp(fmtc->iformat->long_name, "QuickTime / MOV")
                || !strcmp(fmtc->iformat->long_name, "FLV (Flash Video)")
                || !strcmp(fmtc->iformat->long_name, "Matroska / WebM")
            );

        bMp4MPEG4 = eVideoCodec == AV_CODEC_ID_MPEG4 && (
                !strcmp(fmtc->iformat->long_name, "QuickTime / MOV")
                || !strcmp(fmtc->iformat->long_name, "FLV (Flash Video)")
                || !strcmp(fmtc->iformat->long_name, "Matroska / WebM")
            );

        //Initialize packet fields with default values
        av_init_packet(&pkt);
        pkt.data = NULL;
        pkt.size = 0;
        av_init_packet(&pktFiltered);
        pktFiltered.data = NULL;
        pktFiltered.size = 0;

        // Initialize bitstream filter and its required resources
        if (bMp4H264) {
            const AVBitStreamFilter *bsf = av_bsf_get_by_name("h264_mp4toannexb");
            if (!bsf) {
                NV_LOG(ERROR) << "FFmpeg error: " << __FILE__ << " " << __LINE__ << " " << "av_bsf_get_by_name() failed";
                return;
            }
            ck(av_bsf_alloc(bsf, &bsfc));
            avcodec_parameters_copy(bsfc->par_in, fmtc->streams[iVideoStream]->codecpar);
            ck(av_bsf_init(bsfc));
        }
        if (bMp4HEVC) {
            const AVBitStreamFilter *bsf = av_bsf_get_by_name("hevc_mp4toannexb");
            if (!bsf) {
                NV_LOG(ERROR) << "FFmpeg error: " << __FILE__ << " " << __LINE__ << " " << "av_bsf_get_by_name() failed";
                return;
            }
            ck(av_bsf_alloc(bsf, &bsfc));
            avcodec_parameters_copy(bsfc->par_in, fmtc->streams[iVideoStream]->codecpar);
            ck(av_bsf_init(bsfc));
        }
        NV_LOG(INFO) << "eVideoCodec: " << eVideoCodec <<" bMp4H264: " << bMp4H264 << " bMp4HEVC: " << bMp4HEVC;
    }

    AVFormatContext *CreateFormatContext(DataProvider *pDataProvider) {

        AVFormatContext *ctx = NULL;
        if (!(ctx = avformat_alloc_context())) {
            NV_LOG(ERROR) << "FFmpeg error: " << __FILE__ << " " << __LINE__;
            return NULL;
        }

        uint8_t *avioc_buffer = NULL;
        int avioc_buffer_size = 8 * 1024 * 1024;
        avioc_buffer = (uint8_t *)av_malloc(avioc_buffer_size);
        if (!avioc_buffer) {
            NV_LOG(ERROR) << "FFmpeg error: " << __FILE__ << " " << __LINE__;
            return NULL;
        }
        avioc = avio_alloc_context(avioc_buffer, avioc_buffer_size,
            0, pDataProvider, &ReadPacket, NULL, NULL);
        if (!avioc) {
            NV_LOG(ERROR) << "FFmpeg error: " << __FILE__ << " " << __LINE__;
            return NULL;
        }
        ctx->pb = avioc;

        ck(avformat_open_input(&ctx, NULL, NULL, NULL));
        return ctx;
    }

    /**
    *   @brief  Allocate and return AVFormatContext*.
    *   @param  szFilePath - Filepath pointing to input stream.
    *   @return Pointer to AVFormatContext
    */
     AVFormatContext *CreateFormatContext(const char *szFilePath) 
    {
        strcpy(filename, szFilePath);
        //1.进行网络组件的全局初始化
        avformat_network_init();
        AVDictionary* options = NULL;
        //配置参数(如果视频流出现是文件通过live555等推送的，当一个文件结束时，则需要重新配置，打开视频流，不然会一直读帧失败)
        av_dict_set(&options, "rtsp_transport", "tcp", 0);  //默认UDP，容易出现丢包的问题
    
		//av_dict_set(&options, "stimeout", "3000000", 0);    //设置超时断开连接时间
		av_dict_set(&options, "bufsize", "‭2097151", 0);
        av_dict_set(&options, "max_delay", "500000", 0);    //设置最大时延
        av_dict_set(&options, "framerate", "10", 0);

         //2.打开输入视频文件
        AVFormatContext *ctx = NULL;
        ck(avformat_open_input(&ctx, szFilePath, NULL, &options));
        return ctx;
    }

public:
    FFmpegDemuxer(const char *szFilePath) : FFmpegDemuxer(CreateFormatContext(szFilePath)) {}
    FFmpegDemuxer(DataProvider *pDataProvider) : FFmpegDemuxer(CreateFormatContext(pDataProvider)) {avioc = fmtc->pb;}
    ~FFmpegDemuxer() {

        if (!fmtc) {
            return;
        }

        if (pkt.data) {
            av_packet_unref(&pkt);
        }
        if (pktFiltered.data) {
            av_packet_unref(&pktFiltered);
        }

        if (bsfc) {
            av_bsf_free(&bsfc);
        }

        avformat_close_input(&fmtc);

        if (avioc) {
            av_freep(&avioc->buffer);
            av_freep(&avioc);
        }

        if (pDataWithHeader) {
            av_free(pDataWithHeader);
        }
    }
    AVCodecID GetVideoCodec() {
        return eVideoCodec;
    }
    AVPixelFormat GetChromaFormat() {
        return eChromaFormat;
    }
    int GetWidth() {
        return nWidth;
    }
    int GetHeight() {
        return nHeight;
    }
    int GetBitDepth() {
        return nBitDepth;
    }
    int GetFrameSize() {
        return nWidth * (nHeight + nChromaHeight) * nBPP;
    }

    uint64_t GetPframe_interval_pts()
    {
        return Pframe_interval_pts;
    }

    //探测P帧间隔
    int ffprobe()
    {
        NV_LOG(INFO) << "*******Probe interval pts of P frame**********"; 
        pCodecCtx = avcodec_alloc_context3(NULL);
        if (avcodec_parameters_to_context(pCodecCtx, fmtc->streams[iVideoStream]->codecpar) < 0)
        {
            NV_LOG(ERROR) << "FFmpegDemuxer::ffprobe::avcodec_parameters_to_context error";
            return -1;
        }

        pFrame = av_frame_alloc();

        //查找FFmpeg的解码器
        pCodec = avcodec_find_decoder(pCodecCtx->codec_id);
        if(pCodec == NULL)
            NV_LOG(ERROR) <<  "could not find codec id : " << pCodecCtx->codec_id;

        //初始化一个视音频编解码器的AVCodecContext
        if (avcodec_open2(pCodecCtx, pCodec, NULL) < 0)
            NV_LOG(ERROR) <<"could not open codec id : " << pCodecCtx->codec_id;
        
        //获取硬件支持的并发线程数
        pCodecCtx->thread_count = std::thread::hardware_concurrency() > 8 ? 4 : (std::thread::hardware_concurrency() < 2 ? 1 : std::thread::hardware_concurrency() / 2);
        pCodecCtx->thread_safe_callbacks = 1;

        int read_frame_ret, receive_frame_ret;
        int ffprobe_frame_count = 0;
        int receive_frame_count = 0;

        while(1)
        {
            if (pkt.data) 
                av_packet_unref(&pkt);
            //读取一帧数据
            read_frame_ret = av_read_frame(fmtc, &pkt);
            if(read_frame_ret >= 0 && pkt.stream_index != iVideoStream)
            {
                av_packet_unref(&pkt);
                continue;
            }

            ffprobe_frame_count++;

            //发送1帧编码数据给解码器
            if(avcodec_send_packet(pCodecCtx, &pkt))
            {
                NV_LOG(WARNING) << "avcodec_send_packet error";
                continue;
            }
            
            //获取解码器的解码结果，因为解码速度原因，不一定发送了数据给解码器就能接着读到解码器的结果，因此每次发送编码数据给解码器，要读多次，应对可能解码器已经有多帧数据解码完成，且可读
            while(1)
            {
                av_frame_unref(pFrame);
                receive_frame_ret = avcodec_receive_frame(pCodecCtx, pFrame);
                if(receive_frame_ret == AVERROR(EAGAIN) || receive_frame_ret == AVERROR_EOF || Exit_probe)
                    break;
                receive_frame_count++;
                switch(pFrame->pict_type)
                {
                    case AV_PICTURE_TYPE_I:
                        NV_LOG(INFO) << "I_frame: " << receive_frame_count << " and pts:" << pFrame->pts;
                        break;
                    case AV_PICTURE_TYPE_P:
                        if(!start_P_sta)
                        {
                            start_P_sta = true;
                            start_Pframe_pts = pFrame->pts;
                        }
                        else
                        {
                            end_Pframe__pts = pFrame->pts;
                            Pframe_interval_pts = end_Pframe__pts - start_Pframe_pts;
                            Exit_probe = true;
                        }
                        NV_LOG(INFO) << "P_frame: " << receive_frame_count << " and pts:" << pFrame->pts;
                        break;
                    case AV_PICTURE_TYPE_B:
                        NV_LOG(INFO) << "B_frame: " << receive_frame_count << " and pts:" << pFrame->pts;
                        break;
                    case AV_PICTURE_TYPE_S:
                        //NV_LOG(INFO) << "S_frame: " << receive_frame_count << " and pts:" << pFrame->pts;
                        break;
                    case AV_PICTURE_TYPE_SI:
                        //NV_LOG(INFO) << "SI_frame: " << receive_frame_count << " and pts:" << pFrame->pts;
                        break;
                    case AV_PICTURE_TYPE_SP:
                        //NV_LOG(INFO) << "SP_frame: " << receive_frame_count << " and pts:" << pFrame->pts;
                        break;
                    case AV_PICTURE_TYPE_BI:
                        //NV_LOG(INFO) << "BI_frame: " << receive_frame_count << " and pts:" << pFrame->pts;
                        break;
                    default:
                        NV_LOG(INFO) << "Unkonwn frame typpe: " << receive_frame_count << " and pts:" << pFrame->pts;
                }
            }

            if(read_frame_ret < 0)
            {
                NV_LOG(ERROR) << "av_read_frame error or finished " << " and read frames: " << ffprobe_frame_count << " and receive frame: " << receive_frame_count;
                return -1;
            }

            if(Exit_probe)
            {
                if (pFrame) 
                    av_frame_free(&pFrame);
                if (pCodecCtx) 
                    avcodec_free_context(&pCodecCtx);

                //重新打开视频源
                avformat_close_input(&fmtc);
                AVDictionary* options = NULL;
                av_dict_set(&options, "rtsp_transport", "tcp", 0);  //默认UDP，容易出现丢包的问题
                //av_dict_set(&options, "stimeout", "3000000", 0);    //设置超时断开连接时间
                av_dict_set(&options, "bufsize", "‭2097151", 0);
                av_dict_set(&options, "max_delay", "500000", 0);    //设置最大时延
                av_dict_set(&options, "framerate", "10", 0);
                ck(avformat_open_input(&fmtc, filename, NULL, &options));

                NV_LOG(INFO) << "Pframe_interval_pts: " << Pframe_interval_pts;
                NV_LOG(INFO) << "********************Probe End*****************";
                break;
            }
        }

        return 0;
    }

    bool Demux_H264_keyFrame(uint8_t **ppVideo, int *pnVideoBytes, int64_t *pts = NULL)
    {
        bool key_frame = false;
        if (!fmtc) 
            return false;

        *pnVideoBytes = 0;

        int ret = 0;

        while(1)
        {
            if (pkt.data) 
                av_packet_unref(&pkt);

            ret = av_read_frame(fmtc, &pkt);
            if(ret >= 0 && pkt.stream_index != iVideoStream)
            {
                av_packet_unref(&pkt);
                continue;
            }
            else if(ret < 0)
            {
                NV_LOG(ERROR) << "av_read_frame error or finished " << " and total demux frames: " << frameCount;
                return false;
            }

            frameCount++;

            if(eVideoCodec == AV_CODEC_ID_H264)
            {
                //判断是否是关键帧,可以连续解P帧，不能跳着解P帧，B帧依赖性大，容易出问题
                if(pkt.flags == AV_PKT_FLAG_KEY)
                {
                    key_frame = true;
                    key_frameCount++;

                    //需要进行数据过滤
                    if(bMp4H264)
                    {
                        if (pktFiltered.data) 
                        {
                            av_packet_unref(&pktFiltered);
                        }
                        ck(av_bsf_send_packet(bsfc, &pkt));
                        ck(av_bsf_receive_packet(bsfc, &pktFiltered));
                        *ppVideo = pktFiltered.data;
                        *pnVideoBytes = pktFiltered.size;
                        if (pts)
                            *pts = (int64_t) (pktFiltered.pts * 1000 * timeBase);
                    }
                    else
                    {
                        *ppVideo = pkt.data;
                        *pnVideoBytes = pkt.size;
                        if (pts)
                            *pts = (int64_t) (pkt.pts * 1000 * timeBase);
                    }

                    //key_frame_after_count = 4;
                    Kframe_pts = pkt.pts;
                    Pframe_pts = Kframe_pts + Pframe_interval_pts;
                    NV_LOG(INFO) << "key_frameCount: " << key_frameCount << " frameCount: " << frameCount << " and key_frame_size:" << *pnVideoBytes << " and pts:" << pkt.pts << " and dts: " << pkt.dts;
                    break;
                } 
                else
                {
                    key_frame = false;
                    if(pkt.pts == Pframe_pts)
                    {
                        *ppVideo = pkt.data;
                        *pnVideoBytes = pkt.size;
                        if (pts)
                            *pts = (int64_t) (pkt.pts * 1000 * timeBase);
                            
                        NV_LOG(INFO) << "return frame: " <<  frameCount <<" and pts:" << pkt.pts << " and dts: " << pkt.dts;
                        Pframe_pts += Pframe_interval_pts;
                        break;
                    }
                    /*
                    if(key_frame_after_count > 0)
                    {
                        *ppVideo = pkt.data;
                        *pnVideoBytes = pkt.size;
                        key_frame_after_count--;

                        if (pts)
                            *pts = (int64_t) (pkt.pts * 1000 * timeBase);
                            
                        NV_LOG(INFO) << "return frame: " <<  frameCount <<" and pts:" << pkt.pts << " and dts: " << pkt.dts;
                        break;
                    }
                    */
                }
            }
        }

        return true;
    }

    bool Demux(uint8_t **ppVideo, int *pnVideoBytes, int64_t *pts = NULL) 
    {
        bool key_frame = false;
        if (!fmtc) {
            return false;
        }
        *pnVideoBytes = 0;

        if (pkt.data) {
            av_packet_unref(&pkt);
        }

        int e = 0;
        while ((e = av_read_frame(fmtc, &pkt)) >= 0 && pkt.stream_index != iVideoStream)
        {     
            //NV_LOG(INFO) << "e: " << e << " pkt.stream_index:" << pkt.stream_index << " iVideoStream: " << iVideoStream;
            av_packet_unref(&pkt);
        }
        if (e < 0) 
        {
            NV_LOG(ERROR) << "av_read_frame error or finished and demux frame_count: " << frameCount;
            return false;
        }
        
        //需要进行数据过滤
        if (bMp4H264 || bMp4HEVC) 
        {
            if (pktFiltered.data) 
            {
                av_packet_unref(&pktFiltered);
            }
            ck(av_bsf_send_packet(bsfc, &pkt));
            ck(av_bsf_receive_packet(bsfc, &pktFiltered));
            *ppVideo = pktFiltered.data;
            *pnVideoBytes = pktFiltered.size;
            if (pts)
                *pts = (int64_t) (pktFiltered.pts * 1000 * timeBase);
        } 
        else 
        {
            //有额外的数据
            if (bMp4MPEG4 && (frameCount == 0)) 
            {
                int extraDataSize = fmtc->streams[iVideoStream]->codecpar->extradata_size;

                if (extraDataSize > 0) {

                    // extradata contains start codes 00 00 01. Subtract its size
                    pDataWithHeader = (uint8_t *)av_malloc(extraDataSize + pkt.size - 3*sizeof(uint8_t));

                    if (!pDataWithHeader) {
                        NV_LOG(ERROR) << "FFmpeg error: " << __FILE__ << " " << __LINE__;
                        return false;
                    }

                    memcpy(pDataWithHeader, fmtc->streams[iVideoStream]->codecpar->extradata, extraDataSize);
                    memcpy(pDataWithHeader+extraDataSize, pkt.data+3, pkt.size - 3*sizeof(uint8_t));

                    *ppVideo = pDataWithHeader;
                    *pnVideoBytes = extraDataSize + pkt.size - 3*sizeof(uint8_t);
                }

            } 
            else 
            {
                *ppVideo = pkt.data;
                *pnVideoBytes = pkt.size;
            }

            if (pts)
                *pts = (int64_t)(pkt.pts * 1000 * timeBase);
        }

        frameCount++;

        return true;
    }

    static int ReadPacket(void *opaque, uint8_t *pBuf, int nBuf) {
        return ((DataProvider *)opaque)->GetData(pBuf, nBuf);
    }

    /*
    int is_keyframe_h264(AVPacket* pkt) 
    {
        // printf("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
        // pkt->data[0], pkt->data[1], pkt->data[2], pkt->data[3], pkt->data[4],
        // pkt->data[5], pkt->data[6], pkt->data[7], pkt->data[8], pkt->data[9]);
        int length = 0;
        unsigned char* p_buffer = pkt->data;
        int i_buffer = pkt->size;
        if ( (pkt->data[0] == 0 && pkt->data[1] == 0 && pkt->data[2] == 0 && pkt->data[3] == 1) ||
        (pkt->data[0] == 0 && pkt->data[1] == 0 && pkt->data[2] == 1) ) 
        {
            while (i_buffer > 3) 
            {
                if (pkt->data[0] == 0 && pkt->data[1] == 0 && pkt->data[2] == 0 && pkt->data[3] == 1) 
                {
                    p_buffer += 4;
                    i_buffer -= 4;
                    int nal_type = p_buffer[0] & 0x1f;
                    // printf("1 pkt.size %d length %d nal_type %d\n", pkt->size, length, nal_type);
                    if (nal_type == 5) 
                        return 1;
                }
                if (pkt->data[0] == 0 && pkt->data[1] == 0 && pkt->data[2] == 1) 
                {
                    p_buffer += 3;
                    i_buffer -= 3;
                    int nal_type = p_buffer[0] & 0x1f;
                    // printf("2 pkt.size %d length %d nal_type %d\n", pkt->size, length, nal_type);
                    if (nal_type == 5) 
                        return 1;
                }
                p_buffer++;
                i_buffer--;
            }
        } 
        else 
        {
            while (i_buffer > 4) 
            {
                unsigned char* p = (unsigned char*)&length;
                p[3] = p_buffer[0];
                p[2] = p_buffer[1];
                p[1] = p_buffer[2];
                p[0] = p_buffer[3];
                p_buffer += 4;
                i_buffer -= 4;
                int nal_type = p_buffer[0] & 0x1f;
                //printf("3 p_buffer[0] 0x%02x pkt.size %d length %d nal_type %d\n", p_buffer[0], pkt->size, length, nal_type);
                //if (nal_type == 5) return 1;
                return 1;
                p_buffer += length;
                i_buffer -= length;
            }
        }
        // printf("this h264 frame is not keyframe\n");
        return 0;
    }
    int is_keyframe(AVCodec *dec, AVPacket* pkt) 
    {
        if (dec && dec->name) 
        {
            int cpylen = strlen((const char*)dec->name);
            if (cpylen >= 4) 
            {
                if (0 == memcmp(dec->name, "h264", 4)) 
                {
                    return is_keyframe_h264(pkt);
                }
            }
        }
        return 1;
    }
    */
};

inline cudaVideoCodec FFmpeg2NvCodecId(AVCodecID id) {
    switch (id) {
    case AV_CODEC_ID_MPEG1VIDEO : return cudaVideoCodec_MPEG1;
    case AV_CODEC_ID_MPEG2VIDEO : return cudaVideoCodec_MPEG2;
    case AV_CODEC_ID_MPEG4      : return cudaVideoCodec_MPEG4;
    case AV_CODEC_ID_VC1        : return cudaVideoCodec_VC1;
    case AV_CODEC_ID_H264       : return cudaVideoCodec_H264;
    case AV_CODEC_ID_HEVC       : return cudaVideoCodec_HEVC;
    case AV_CODEC_ID_VP8        : return cudaVideoCodec_VP8;
    case AV_CODEC_ID_VP9        : return cudaVideoCodec_VP9;
    case AV_CODEC_ID_MJPEG      : return cudaVideoCodec_JPEG;
    default                     : return cudaVideoCodec_NumCodecs;
    }
}


