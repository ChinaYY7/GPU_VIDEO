...
public:
    ...
    char* GetLead(LogLevel l, const char *szFile, int nLine, const char *szFunc) {
        ...
        const char *szLevels[] = {"NV_TRACE", "NV_INFO", "NV_WARN", "NV_ERROR", "NV_FATAL"};
        if (bPrintTimeStamp) {
            time_t t = time(NULL);
...
#define NV_LOG(level) simplelogger::LogTransaction(logger, level, __FILE__, __LINE__, __FUNCTION__).GetStream()
