...
{
    START_TIMER
    NV_LOG(INFO)<<"HandleVideoSequence::Create or change decoder";
    //获取解码的视频源信息
    m_videoInfo.str("");
    m_videoInfo.clear();
    ...
    m_videoInfo << "\nVideo Decoding Params:" << std::endl
...
int NvDecoder::HandlePictureDecode(CUVIDPICPARAMS *pPicParams) 
{
    ...
    //NV_LOG(INFO)<< "HandlePictureDecode::CurrPicIdx: " << pPicParams->CurrPicIdx << " ready to decode";
...
{
    NV_LOG(INFO)<< "HandlePictureDisplay::picture_index: " << pDispInfo->picture_index << " finish decode" << " and timestamp: " << pDispInfo->timestamp;
...
NvDecoder::~NvDecoder() {
    ...
    {
        //NV_LOG(WARNING) << "nFrameAlloc(" << m_nFrameAlloc << ") != m_vpFrame.size()(" << m_vpFrame.size() << ")";
    }
...
bool NvDecoder::Decode(const uint8_t *pData, int nSize, uint8_t ***pppFrame, int *pnFrameReturned, uint32_t flags, int64_t **ppTimestamp, int64_t timestamp, CUstream stream)
{
    ...
    //NV_LOG(INFO)<<"Decode thread id: "<< std::this_thread::get_id();
    m_nDecodedFrame = 0;
    ...
    //NV_LOG(INFO) << "Start cuvidParseVideoData";
    if (m_pMutex) m_pMutex->lock();
    NVDEC_API_CALL(cuvidParseVideoData(m_hParser, &packet));
    if (m_pMutex) m_pMutex->unlock();
    //NV_LOG(INFO) << "Finish cuvidParseVideoData";
    m_cuvidStream = 0;
