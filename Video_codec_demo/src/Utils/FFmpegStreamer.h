...
public:
    FFmpegStreamer(AVCodecID eCodecId, int nWidth, int nHeight, int nFps, const char *szInFilePath) : nFps(nFps) {
        ...
        if (!oc) {
            NV_LOG(ERROR) << "FFMPEG: avformat_alloc_context error";
            return;
        }
        ...
        NV_LOG(INFO) << "Streaming destination: " << oc->url;

        // Add video stream to oc
        vs = avformat_new_stream(oc, NULL);
        if (!vs) {
            NV_LOG(ERROR) << "FFMPEG: Could not alloc video stream";
            return;
        }
        vs->id = 0;
        ...
        if (avio_open(&oc->pb, oc->url, AVIO_FLAG_WRITE) < 0) {
            NV_LOG(ERROR) << "FFMPEG: Could not open " << oc->url;
            return ;
        }

        // Write the container header
        if (avformat_write_header(oc, NULL)) {
            NV_LOG(ERROR) << "FFMPEG: avformat_write_header error!";
            return;
        }
    }
    ~FFmpegStreamer() {
    ...
    bool Stream(uint8_t *pData, int nBytes, int nPts) {
        ...
        if (ret < 0) {
            NV_LOG(ERROR) << "FFMPEG: Error while writing video frame";
        }
