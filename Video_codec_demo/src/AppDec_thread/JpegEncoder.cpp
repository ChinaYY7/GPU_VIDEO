...
namespace NvCodec
{
	...
	int JpegEncoder::init()
	{
		...
		{   
			LOG(ERROR) << "In PostProcess::saveJpeg(): gpujpeg_init_device Fail!  and  jpegDevice_" << jpegDevice_ << " errorcode: " << ret;
			return -1;
		}

		pEncoder_  = gpujpeg_encoder_create(NULL);
		assert(pEncoder_ != NULL);
		LOG(WARNING) << "Jpeg_GPU init success ";
	}		
	...
	void *JpegEncoder::run(void *arg)
	{
		...
		while (1)
		{
			...
			//printf("jpeg encode time: %d us\n", util::timer::getDuration(startTime, endTime));
