...
inline bool check(CUresult e, int iLine, const char *szFile) {
    if (e != CUDA_SUCCESS) {
        ...
        NV_LOG(FATAL) << "CUDA driver API error " << szErrName << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(cudaError_t e, int iLine, const char *szFile) {
    if (e != cudaSuccess) {
        NV_LOG(FATAL) << "CUDA runtime API error " << cudaGetErrorName(e) << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(NVENCSTATUS e, int iLine, const char *szFile) {
    ...
    if (e != NV_ENC_SUCCESS) {
        NV_LOG(FATAL) << "NVENC error " << aszErrName[e] << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(HRESULT e, int iLine, const char *szFile) {
    if (e != S_OK) {
        NV_LOG(FATAL) << "HRESULT error 0x" << (void *)e << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(GLenum e, int iLine, const char *szFile) {
    if (e != 0) {
        NV_LOG(ERROR) << "GLenum error " << e << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(int e, int iLine, const char *szFile) {
    if (e < 0) {
        NV_LOG(ERROR) << "General error " << e << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
    ...
    BufferedFileReader(const char *szFileName, bool bPartial = false) {
        ...
        while (nSize) {
            try {
                ...
                if (nSize != st.st_size) {
                    NV_LOG(WARNING) << "File is too large - only " << std::setprecision(4) << 100.0 * nSize / st.st_size << "% is loaded"; 
                }
                break;
            } catch(std::bad_alloc) {
                if (!bPartial) {
                    NV_LOG(ERROR) << "Failed to allocate memory in BufferedReader";
                    return;
                }
        ...
        if (!fpIn)
        {
            NV_LOG(ERROR) << "Unable to open input file: " << szFileName;
            return;
        }
